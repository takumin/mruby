...
/* A NaN is equal to nothing at all, its own operand included, so `==` can
   ...
   object to go by. Each NaN takes a count of its own in the payload, which no
   arithmetic reads; `mrb_obj_eq()` compares what a Float holds bit for bit so
   that a NaN is the same object as itself and no other. */
