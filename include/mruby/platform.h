 ...
 * `arm64` on darwin and Windows but `aarch64` elsewhere, and the same x86_64
 * is `x64` on Windows.  Code that reads this constant matches it against a
 * pattern (`/mingw|mswin/`, `/darwin/`), so agreeing with the spelling in
 * use beats being internally consistent.
 *
