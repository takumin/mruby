...
assert('String case conversion - a frozen receiver') do
  # Only upcase! reaches the walk's own mrb_check_frozen() on its own: the
  # other three raise from str_replace() further in, so they answer
  # FrozenError whether or not that check is there and pin nothing about it.
