...
- Narrows the character classification of mruby-encoding back to ASCII
  ...
  ASCII to step.
- Drops the Unicode tables the build would otherwise carry, mruby-encoding's
  case table, mruby-regexp's type table and mruby-string-ext's table of the
  letters and the digits. That is what the option is for: a target counting
  its bytes buys the UTF-8 indexing of mruby-encoding without the tables beside
  it.
