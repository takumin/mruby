...
Every build compiles the sources by their names in the tree, and maps the
tree to `.` and the build directory to `build` with `-ffile-prefix-map`. Two
checkouts of the same commit therefore compile the same thing, and a compiler
cache such as `ccache` can share the objects between them.
...
debugger where the sources are (`set substitute-path . /path/to/mruby` in
gdb). A compiler without `-ffile-prefix-map` (`cl`, GCC before 8, clang
...
- create `build/i386/mrblib/mrblib.c` by compiling all `*.rb` files under `mrblib` with native `build/host/mrbc/bin/mrbc`
