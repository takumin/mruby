...
{
  struct RData *d = mrb_data_object_alloc(mrb, mrb->object_class, NULL, &gv_virtual_type);
...
/* iv_put() for the globals table, one search for the whole write: an
 * ordinary name is stored in place, a fresh name inserted, and a virtual
 * name is never overwritten, its dispatch pair handed back instead for
 * the caller to route the value through. */
static const struct gv_virtual*
