 ...
 * the same for `+` on a String receiver, and `OP_STRFRZ` answers `freeze` and
 * `-@` on a string literal with a shared frozen string.  They may only do so
 * while those classes still carry the builtin the opcode reimplements, so each
 * (class, operator) pair keeps a slot in `mrb->idx_class` that holds the class
 * while that is true and NULL once it is not.  The index opcodes compare the
 * receiver's class against the slot instead of against the core class, so a
 * disarmed slot sends the operator like any other method and the check costs
 * nothing while nothing is redefined; `OP_STRFRZ` has a literal for a receiver
 * and reads only whether the slot is armed.
 *
