  ...
  INIT_DISPATCH {
    ...
    CASE(OP_LOADL, BB) {
      if ((irep->pool[b].tt & IREP_TT_NFLAG) == 0) {
        /* a string literal that its file froze, which `L_FROZEN_LITERAL`
           answers with the one string of its text */
    ...
    L_FROZEN_LITERAL:
#ifndef MRB_NO_FRZSTR_CACHE
      {
        ...
        mrb_value str = mrb_str_frozen_literal(mrb, irep, b);
        regs[a] = str;
        sc->irep = irep;
        sc->idx = b;
...
#else
      regs[a] = mrb_str_frozen_literal(mrb, irep, b);
#endif
      mrb_gc_arena_restore(mrb, ai);
      NEXT;
    }
