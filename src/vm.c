...
/*
 * Gives `ci` a proc for the C function it is running, so that entering that
 * function again is the same call however the send moves the method table
 * under it.  Done once, when the frame is first handed back.
 *
 * mrb_define_method() stores a bare mrb_func_t with no RProc behind it, so a
 * frame running such a method has `proc == NULL` and holds nothing that can
 * be called a second time.  Naming the method by the receiver's class and the
 * method name instead, and looking that up on re-entry, would read whatever
 * the send left in the method table rather than the method that is running:
 * a `==` that redefined the walk sending it would have the walk carry on in
 * the new definition, where CRuby and a C function that stays on the C stack
 * carry on in the old one.
 */
static void
cont_proc_set(mrb_state *mrb, mrb_callinfo *ci)
{
  if (ci->proc) {
  ...
  if (MRB_METHOD_UNDEF_P(m) || !MRB_METHOD_CFUNC_P(m)) {
    mrb_raisef(mrb, E_RUNTIME_ERROR, "'%n' is not the C function running it", ci->mid);
  }
  struct RProc *p = mrb_proc_new_cfunc(mrb, MRB_METHOD_CFUNC(m));
  ci = mrb->c->ci;
  CI_PROC_SET(ci, p);
}
...
static mrb_func_t
cont_func(mrb_callinfo *ci)
{
  mrb_assert(ci->proc && MRB_PROC_CFUNC_P(ci->proc));
...
mrb_funcall_k(mrb_state *mrb, mrb_value recv, mrb_sym mid, mrb_int argc, const mrb_value *argv, mrb_value state)
{
  ...
  if (!MRB_CI_CONT_P(ci)) {
    cont_proc_set(mrb, ci);     /* the first time this frame is handed back */
    ci = mrb->c->ci;
  }
...
#undef DISPATCH_CHECKPOINTS
    ...
    L_SENDB_SYM:
    {
      ...
      L_CFUNC_RESUME:
        ...
        recv = cont_func(ci)(mrb, ci->stack[0]);
      }
