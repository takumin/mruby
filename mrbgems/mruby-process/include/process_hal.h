 ...
 * Reading a name is one shape too: a port declares the user table and the
 * group table one at a time, as it declares the calls, and owes the one
 * lookup function as soon as it declared either.  A method whose table the
 * port did not declare takes its ID by number alone.
 *
 * Derived here from the port's macros, one a table or a call, and not the
 * port's to define.
 */
...
# define MRB_HAL_PROCESS_NEEDS_ID_BY_NAME
#endif
 ...
 * apart, as it does for getpwnam(3) itself.  Which of the two an unknown
 * name is is the table's answer to report and not the port's to arrange:
 * a port passes on what it was told rather than reading a failure as a name
 * that was not there.
 *
 * Owed by a port that declared MRB_HAL_PROCESS_HAS_UID_BY_NAME or
 * MRB_HAL_PROCESS_HAS_GID_BY_NAME beside a call taking an ID, and asked
 * only for a `kind` whose table the port declared: for the other the common
 * layer refuses a name where it refuses anything that is not an Integer,
 * with a TypeError, which is what CRuby built without <pwd.h> answers.
 *
