 ...
 * mrb_process_clock_time, for the reason given above it: the HAL cannot be
 * typed in mrb_float, since that would not compile under MRB_NO_FLOAT, and a
 * fixed-width count of ticks would either lose whatever a platform reads
 * finer than it or overflow on a process that has run long enough.  Turning
 * the four into a Ruby Float, and building the Process::Tms they are
 * answered as, is Process.times's job once every port has answered the same
 * shape; a port is asked for nothing beyond the four readings themselves.
 */
 ...
 * a child's CPU time (Windows, which has no wait of its own here yet in any
 * case) answers 0 for both, which is an honest reading of nothing having
 * been added to them rather than a guess.
 *
