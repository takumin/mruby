...
**   - `Process.times`' cutime and cstime read 0.  Win32 has no call that
**     answers a reaped child's CPU time, and this port creates no children
**     in any case, so there is nothing to add up; 0 says exactly that
...
/* The kernel and user FILETIMEs behind a process's or a thread's CPU time,
   ...
   mrb_hal_process_times() keeps them apart as utime and stime. Splitting
   this out is what keeps the GetProcessTimes()/GetThreadTimes() call and its
   error handling in one place for both. */
static int
