 ...
 * The wider calls are taken as a pair or not at all, unlike the two calls
 * the methods are declared by: a limit is read and written in the one type
 * below, so a host with one of the two and not the other reads and writes
 * through the narrower pair.
 */
...
/* This host's spelling of the limits that are not numbers, in the type chosen
   above.  Linux, the BSDs and macOS spell their saved limits the way they
   spell no limit; illumos gives each its own value. */
static const mrb_process_rlimit_spelling rlimit_spelling = {
  (port_rlim_t)-1,
  PORT_RLIM_INFINITY,
#ifdef PORT_RLIM_SAVED_CUR
  PORT_RLIM_SAVED_CUR, TRUE,
#else
  0, FALSE,
#endif
#ifdef PORT_RLIM_SAVED_MAX
  PORT_RLIM_SAVED_MAX, TRUE,
#else
  0, FALSE,
#endif
};
...
mrb_hal_process_getrlimit(mrb_state *mrb, mrb_int id, mrb_process_rlimit *r)
{
  ...
  mrb_process_rlimit_from_platform(&rlimit_spelling, (uint64_t)rlim.rlim_cur, &r->cur);
  mrb_process_rlimit_from_platform(&rlimit_spelling, (uint64_t)rlim.rlim_max, &r->max);
  return 0;
}
#endif
...
mrb_hal_process_setrlimit(mrb_state *mrb, mrb_int id, const mrb_process_rlimit *r)
{
  ...
  if (resource == MRB_RLIMIT_ABSENT ||
      mrb_process_rlimit_to_platform(&rlimit_spelling, &r->cur, &cur) != 0 ||
      mrb_process_rlimit_to_platform(&rlimit_spelling, &r->max, &max) != 0) {
    errno = EINVAL;
    return -1;
  }
  rlim.rlim_cur = (port_rlim_t)cur;
  rlim.rlim_max = (port_rlim_t)max;
