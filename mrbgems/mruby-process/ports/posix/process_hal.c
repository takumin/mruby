...
** fork(2)/exec(3), waitpid(2) and kill(2).  The clocks are clock_hal.c's.
...
/* setrlimit(2) and the resource names, which this port declares itself able
   to set only where mrbgem.rake found the header they are declared in. */
...
/* Whether this host has a posix_spawn() that answers for the exec.
   ...
   failed returns 0, and so does one under qemu's user-mode emulation, which
...
#endif /* MRB_HAL_PROCESS_HAS_WAIT */
