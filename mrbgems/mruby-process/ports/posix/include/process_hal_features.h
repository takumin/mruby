...
**
** One macro a call, one more for reading a name, and none of them names a
** platform.  Which of the `Process::Sys` calls a host has is asked of its
** compiler and linker by mrbgem.rake, whether <unistd.h> declares the call
** and the C library defines it (check_func), and answered here as HAVE_*,
** one a call as CRuby's configure has them.
...
/* getpwnam_r(3) and getgrnam_r(3): a name in place of a number, in every
   `Process::Sys` method that takes an ID.  Not a call of its own but what
   the port reads names with, declared only with both, since a name can
   stand for a user or a group alike.  Without it a method takes numbers
   alone and a name is a TypeError, as CRuby's built without <pwd.h>
   answers.  Every host with the setters has had the two since POSIX.1-2001,
   and they are asked about rather than assumed for the same reason the
   calls are. */
#if defined(HAVE_GETPWNAM_R) && defined(HAVE_GETGRNAM_R)
# define MRB_HAL_PROCESS_HAS_ID_BY_NAME
#endif
