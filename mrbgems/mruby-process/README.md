...
ID is one macro more, `MRB_HAL_PROCESS_HAS_ID_BY_NAME`, which a port declares
beside the calls when it has an account table to read; without it a method
takes numbers alone and a name is the `TypeError` anything but an Integer is,
as CRuby built without `<pwd.h>` answers.

The POSIX port names no platform: `mrbgem.rake` asks the compiler and the
linker about each call (`check_func`) and writes `HAVE_<CALL>` where the host
has it, and about `getpwnam_r(3)` and `getgrnam_r(3)` the same way, and the
port's feature header reads that. The posix column below says where the probe
answers yes.
...
| `MRB_HAL_PROCESS_HAS_ID_BY_NAME` | a name in any of the setters                      | o                                  |     |
...
- Whether `<sys/resource.h>` exists, which of the fifteen credential calls
  ...
  unimplemented, and one without the lookups takes an ID by number alone.
...
- On Windows `Process::Tms#cutime` and `#cstime` always read `0.0`: Win32
  ...
  same way.
- A user or group name no account has is `ArgumentError` from every
  `Process::Sys` method that takes a name. CRuby raises the same error only
  when `getpwnam_r(3)` returned 0 with no record, and reports the `ENOENT`
  glibc answers for an unknown name as `Errno::ENOENT` instead; here every
  value `getpwnam(3)` lists for a name that was not found, `0`, `ENOENT`,
  `ESRCH`, `EBADF` and `EPERM`, is read as the unknown name it means, the same
  five CRuby's `pwd_not_found` reads, and any other failure is an `Errno::*`,
  naming the account it was looking for.
