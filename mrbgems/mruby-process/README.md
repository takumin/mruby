...
| Process.waitpid                   | o             | sets `$?`; if the port waits, see below  |
...
- On Windows only `KILL` and `TERM` can be delivered (as
  ...
  and nothing more, so a status always reads as exited, and the port declares
  no wait until `Process.spawn` exists to make children;
