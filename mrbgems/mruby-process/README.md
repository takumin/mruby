...
- Which kind a platform's number is, and which number a kind is handed down
  as, is the common layer's to decide from the spelling a port gives it, so a
  number the platform spells another kind as is refused with `EINVAL` rather
  than set as that kind (`mrb_process_rlimit_to_platform` in `src/rlimit.c`).
...
- The POSIX port reads and writes through `getrlimit64(2)` where its own
  `rlim_t` is too narrow and the host has both wider calls; an illumos 32-bit
...
- `Process::RLIM_INFINITY` is `-1`, where CRuby answers with the platform's own
  number, of which there is no one spelling: with a 64-bit `rlim_t` Linux
  writes 18446744073709551615, the BSDs and macOS 9223372036854775807, illumos
  `(rlim_t)-3`. Linux's is past `mrb_int` in every build, and past every Integer
  of a build without `mruby-bigint`, so answering with it would leave
  `Process.getrlimit` raising `RangeError` for an ordinary machine's unlimited
  resources.
...
- A number the platform spells no limit or a saved limit as raises
  `Errno::EINVAL`: with a 64-bit `rlim_t`, 18446744073709551615 on Linux and
  9223372036854775807 on the BSDs and macOS. CRuby hands it to the platform,
  which sets no limit, so a limit written as a number reads back there as
