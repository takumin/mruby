...
- **`waitpid` returns a raw status, decoded separately.** `Process::Status`
  ...
  what the `mruby-io` integration is — so decoding has to stand on its own
  either way. Having the wait return a decoded struct as well would be a
...
- `Process.kill` does not signal process groups yet. A negative signal number,
  ...
  quietly signalling the process instead.
