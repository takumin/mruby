...
A port declares in the same header which `Process.spawn` options it acts on;
those macros and what each one stands for are in "Creating a child" below.
...
should set `errno` to `ENOSYS` — or `ENOTSUP` for a redirection it cannot
express — and return the documented failure value rather than pretending to
succeed; something it cannot do at all it leaves undeclared.
