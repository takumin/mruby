MRuby::Gem::Specification.new('mruby-process') do |spec|
  ...
  spec.build_settings do |spec|
    ...
    # dropped does not count.  The one answer taken back is NetBSD's for
    # setruid(2) and setrgid(2), in the port's feature header, which says why.
    ...
    # host is not a name either, so the two are asked the same way, and each
    # answers for its own table: without one, the setters reading that table
    # take IDs by number alone, as CRuby's do when it was built without
    # <pwd.h>.
