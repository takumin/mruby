...
** `uint64_t` and the kind of answer it is and a resource as one of mruby's
** own numbers, and everything Ruby says about them is said here: which
** resources can be named, how a name is read, which numbers name no limit,
** and which of the `Process::RLIMIT_*` constants this build defines.  A port
** answers what its platform has and is asked nothing about any of that, and
** it tells a platform's number from its spelling of the other kinds with the
** two conversions here, which it hands that spelling to.
*/
...
/* The answers a limit can be instead of a number, which is what the kinds the
   ...
   borrow in any case: where `rlim_t` is 64 bits wide, no limit is 2**64 - 1
   on Linux, 2**63 - 1 on the BSDs and macOS and `((rlim_t)-3)` on illumos,
   and the first of those is past `mrb_int` in every build.  CRuby answers
   with the platform's numbers instead, and so defines `RLIM_SAVED_MAX` as
   `RLIM_INFINITY` where a host spells the two the same; here the three are
   distinct whatever the host does, and a host that cannot tell them apart
   simply never answers with a saved one. */
...
void
mrb_process_rlimit_from_platform(const mrb_process_rlimit_spelling *s, uint64_t v,
                                 mrb_process_rlimit_value *out)
{
  ...
  else if (s->has_saved_cur && v == s->saved_cur) {
    out->kind = MRB_PROCESS_RLIMIT_SAVED_CUR;
  }
  else if (s->has_saved_max && v == s->saved_max) {
...
int
mrb_process_rlimit_to_platform(const mrb_process_rlimit_spelling *s,
                               const mrb_process_rlimit_value *v, uint64_t *out)
{
  switch (v->kind) {
    case MRB_PROCESS_RLIMIT_VALUE:
      if (v->value > s->widest || v->value == s->infinity ||
          (s->has_saved_cur && v->value == s->saved_cur) ||
          (s->has_saved_max && v->value == s->saved_max)) {
        return -1;
      }
      *out = v->value;
      return 0;
    ...
    case MRB_PROCESS_RLIMIT_SAVED_CUR:
      if (!s->has_saved_cur) return -1;
      *out = s->saved_cur;
      return 0;
    case MRB_PROCESS_RLIMIT_SAVED_MAX:
      if (!s->has_saved_max) return -1;
      *out = s->saved_max;
      return 0;
    default:
      return -1;
 ...
 * Errno::EINVAL for a soft limit above the hard one or for a number the
 * platform spells no limit or a saved limit as.  A negative limit other than
 * the three constants raises RangeError.
 */
static mrb_value
