...
static const mrb_process_status*
status_ptr(mrb_state *mrb, mrb_value status)
{
...
{
  return status_ptr(mrb, status)->pid;
}
...
mrb_process_status_new(mrb_state *mrb, const mrb_process_status *status)
{
  ...
  *copy = *status;
  data->data = copy;
  /* What the process did is over by the time there is a status for it, and
     nothing here writes one again; CRuby freezes the status it leaves in $?
     for the same reason. */
  return mrb_obj_freeze(mrb, mrb_obj_value(data));
}

static mrb_value
status_flag(mrb_state *mrb, mrb_value self, unsigned int flag)
{
  return mrb_bool_value((status_ptr(mrb, self)->flags & flag) != 0);
}
...
{
  return mrb_int_value(mrb, status_ptr(mrb, self)->raw_status);
}
...
{
  const mrb_process_status *st = status_ptr(mrb, self);
...
{
  mrb_value raw = mrb_int_value(mrb, status_ptr(mrb, self)->raw_status);
  mrb_value other;
  ...
  if (mrb_obj_class(mrb, self) == mrb_obj_class(mrb, other)) {
    other = mrb_int_value(mrb, status_ptr(mrb, other)->raw_status);
  }
...
{
  const mrb_process_status *st = status_ptr(mrb, self);
  mrb_value str = mrb_str_new_lit(mrb, "pid ");
