 ...
 * A name no account has is an ArgumentError rather than a failed call: the
 * lookup never reaches the call the method stands for, so there is no errno
 * for it to have failed with.  A lookup that failed before it could answer
 * is a different thing, and is reported as the SystemCallError it was,
 * naming the account it was looking for.  On a port that declared no way
 * to read a name (MRB_HAL_PROCESS_HAS_ID_BY_NAME) a String is what anything
 * but an Integer is, a TypeError, as CRuby built without <pwd.h> answers.
 */
static int64_t
 ...
 *
 * A build whose Integer is narrower than an int64_t holds an ID past it as a
 * bigint where it has them, and where it has none there is no number to
 * answer with, which is refused the way a clock reading past an Integer is.
 */
static mrb_value
id_value(mrb_state *mrb, int64_t id, mrb_process_id_kind kind)
{
  (void)kind;
#if MRB_INT_MAX < INT64_MAX
  if (id > MRB_INT_MAX || id < MRB_INT_MIN) {
# ifdef MRB_USE_BIGINT
    return mrb_bint_new_int64(mrb, id);
# else
    mrb_raisef(mrb, E_RANGE_ERROR, "%s does not fit an Integer",
               (kind == MRB_PROCESS_ID_USER) ? "uid" : "gid");
# endif
  }
#endif
...
static mrb_value
getid(mrb_state *mrb, mrb_process_op op, mrb_process_id_kind kind)
{
  int64_t id;
...
{
  return getid(mrb, MRB_PROCESS_OP_GETUID, MRB_PROCESS_ID_USER);
}
#else
...
{
  return getid(mrb, MRB_PROCESS_OP_GETEUID, MRB_PROCESS_ID_USER);
}
#else
...
{
  return getid(mrb, MRB_PROCESS_OP_GETGID, MRB_PROCESS_ID_GROUP);
}
#else
...
{
  return getid(mrb, MRB_PROCESS_OP_GETEGID, MRB_PROCESS_ID_GROUP);
}
#else
