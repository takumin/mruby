...
table_dispose(mrb_state *mrb, mrb_process_table *table)
{
  while (table->live != NULL) {
    ...
    /* A child that has finished is reaped; one that has not, or a wait that
       could not be performed at all, leaves the reap unpaid.  A port that
       declares no wait pays none of them.  Either way the record is let go
       of, which is what keeps this loop finite. */
