...
assert('Process.times and reaped children') do
  ...
  # through waitpid makes cutime + cstime increase strictly. This is the
  # part of the API a HAL bug could get wrong silently: times(2) documents
  # that a waited-for terminated child's CPU time is added to
  # tms_cutime/tms_cstime only once it is waited for.
  skip "this build has no Float" unless ProcessTestUtil.float?
  skip ProcessTestUtil.child_reason if ProcessTestUtil.child_reason
