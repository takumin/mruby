...
assert('Process::Sys takes an ID by name') do
  # The name is looked up in whatever the platform keeps its accounts in.  A
  # table that answers it has no such record raises an ArgumentError, since
  # nothing was asked of the system and there is no errno for the call to have
  # failed with; one that reports a name it has no record of as a failure
  # raises that failure, which is a glibc build here and in CRuby alike.  So
  # what is asserted is the pair rather than one of them: an unknown name is
  # refused, and refused as one of the two.  The user table and the group
  # table are read by two calls a port declares one at a time, so each method
  # is asked about the table its ID comes from; where the port declared none,
  # that method takes numbers alone and a String is the TypeError anything but
  # an Integer is.
  skip 'Process::Sys.setuid is not available' unless Process::Sys.respond_to?(:setuid)

  unknown = lambda do |kind, m, name|
    e = begin
      Process::Sys.send(m, name)
      nil
    rescue ArgumentError, SystemCallError => err
      err
    end
    assert_true !e.nil?, "#{m} took a name no account has"
    assert_equal "can't find #{kind} for #{name}", e.message if e.is_a?(ArgumentError)
  end

  if ProcessSysTest.port_user_names?
    unknown.call('user', :setuid, 'nosuchuser__mruby')
  else
    assert_raise(TypeError) { Process::Sys.setuid('nosuchuser__mruby') }
  end

  if Process::Sys.respond_to?(:setgid)
    if ProcessSysTest.port_group_names?
      unknown.call('group', :setgid, 'nosuchgroup__mruby')
    else
      assert_raise(TypeError) { Process::Sys.setgid('nosuchgroup__mruby') }
    end
  end
  true
end
...
assert('Process::Sys resolves a name it does know') do
  ...
  skip 'this port reads no user name' unless ProcessSysTest.port_user_names?
  skip 'not running as root' unless ProcessSysTestUtil.privileged?
  real = Process::Sys.getuid == 0
  args = real ? ['root', -1] : [-1, 'root']
  # Either way the lookup refuses an unknown name is a system without root.
  skip 'this system has no account named root' if
    (begin; Process::Sys.setreuid(*args); nil; rescue ArgumentError, SystemCallError; true; end)
