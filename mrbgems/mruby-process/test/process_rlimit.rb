...
# so every test here restores what it found, asks for something the system
# refuses, or makes its change in a child through ProcessTestUtil.isolated.
# RLIMIT_CORE is the resource they work on: the size of a core file this
# process would leave behind, which nothing else in the suite reads.  A hard
# limit lowered cannot be raised again by a process that is not privileged,
# so only a child lowers one.
...
assert('Process.setrlimit sets the hard limit to the soft one by default') do
  skip "this port declares no getrlimit" unless ProcessTestUtil.getrlimit?
  skip "this port declares no setrlimit" unless ProcessTestUtil.setrlimit?
  skip "no child to lower a hard limit in" unless ProcessTestUtil.isolated?

  # Left out, or given as nil, max_limit is cur_limit, which lowers a hard
  # limit above it.  Each is asked for in a child, which the lowered limit
  # ends with.
  cur, max = Process.getrlimit(:CORE)
  skip "this process may not leave a core file of any size" if max == 0
  assert_equal "[nil, [0, 0]]", ProcessTestUtil.isolated {
    [Process.setrlimit(:CORE, 0), Process.getrlimit(:CORE)]
  }
  assert_equal "[nil, [0, 0]]", ProcessTestUtil.isolated {
    [Process.setrlimit(:CORE, 0, nil), Process.getrlimit(:CORE)]
  }
  assert_equal [cur, max], Process.getrlimit(:CORE)
end

assert('Process.setrlimit writes a hard limit that is a number') do
  skip "this port declares no getrlimit" unless ProcessTestUtil.getrlimit?
  skip "this port declares no setrlimit" unless ProcessTestUtil.setrlimit?
  skip "no child to lower a hard limit in" unless ProcessTestUtil.isolated?

  # From the largest number this build's Integer writes and the platform
  # holds as a count down to one every build writes, each lowering the hard
  # limit the one before it left.  The first is past `mrb_int` wherever there
  # are bigints and `rlim_t` is 2**64 - 1 wide, and so reaches the platform
  # through a bigint.
  cur, max = Process.getrlimit(:CORE)
  infinity = ProcessRlimitTest.native_infinity
  limits = []
  %w[9223372036854775808 1099511627776 4096].each do |decimal|
    next unless ProcessClockTest.fits?(decimal)
    next unless decimal.size < infinity.size || (decimal.size == infinity.size && decimal < infinity)
    limit = decimal.to_i
    limits << limit if max == Process::RLIM_INFINITY || limit <= max
  end
  skip "this process may not leave a core file of 4096 bytes" if limits.empty?

  assert_equal limits.map { |limit| [0, limit] }.inspect, ProcessTestUtil.isolated {
    limits.map do |limit|
      Process.setrlimit(:CORE, 0, limit)
...
assert('Process.setrlimit does not raise a hard limit') do
  skip "this port declares no getrlimit" unless ProcessTestUtil.getrlimit?
  skip "this port declares no setrlimit" unless ProcessTestUtil.setrlimit?
  skip "no child to lower a hard limit in" unless ProcessTestUtil.isolated?
  skip "this process may raise a hard limit" if ProcessRlimitTest.privileged?

  # A process that is not privileged may lower a hard limit and never raise
  # it again, to a number or to no limit, which the platform refuses with
  # the errno for an operation not permitted.
  assert_equal "Errno::EPERM", ProcessTestUtil.isolated {
    Process.setrlimit(:CORE, 0, 0)
    Process.setrlimit(:CORE, 0, 1)
  }
  assert_equal "Errno::EPERM", ProcessTestUtil.isolated {
    Process.setrlimit(:CORE, 0, 0)
    Process.setrlimit(:CORE, :INFINITY, :INFINITY)
  }
end
...
assert('Process.setrlimit takes the saved limits by name') do
  ...
  # refuses them through errno.  Those hosts cannot tell the two saved limits
  # apart, so which number each reaches the platform as is pinned below, with
  # the spellings written out; here the names are shown to reach a port, and
  # an unknown one to be turned down before anything is asked of the system.
  cur, max = Process.getrlimit(:CORE)
...
assert('a platform number is read as the kind it spells') do
  skip "this port declares no limit call" unless ProcessTestUtil.getrlimit? || ProcessTestUtil.setrlimit?

  top = "18446744073709551615"
  # Each kind spelled apart, the saved limits beside no limit.
  apart = [top, "18446744073709551613", "18446744073709551614", top]
  assert_equal [:INFINITY, nil], ProcessRlimitTest.from_platform("18446744073709551613", apart)
  assert_equal [:SAVED_CUR, nil], ProcessRlimitTest.from_platform("18446744073709551614", apart)
  assert_equal [:SAVED_MAX, nil], ProcessRlimitTest.from_platform(top, apart)
  assert_equal [:VALUE, "18446744073709551612"],
               ProcessRlimitTest.from_platform("18446744073709551612", apart)
  assert_equal [:VALUE, "0"], ProcessRlimitTest.from_platform("0", apart)

  # All three spelled alike, which reads as no limit.
  alike = [top, "9223372036854775807", "9223372036854775807", "9223372036854775807"]
  assert_equal [:INFINITY, nil], ProcessRlimitTest.from_platform("9223372036854775807", alike)
  assert_equal [:VALUE, top], ProcessRlimitTest.from_platform(top, alike)

  # No saved limits named, which leaves their numbers counts.
  unnamed = [top, top, nil, nil]
  assert_equal [:INFINITY, nil], ProcessRlimitTest.from_platform(top, unnamed)
  assert_equal [:VALUE, "18446744073709551614"],
               ProcessRlimitTest.from_platform("18446744073709551614", unnamed)
end

assert('a limit is handed to a platform as the number it spells that kind') do
  skip "this port declares no limit call" unless ProcessTestUtil.getrlimit? || ProcessTestUtil.setrlimit?

  top = "18446744073709551615"
  apart = [top, "18446744073709551613", "18446744073709551614", top]
  assert_equal "18446744073709551613", ProcessRlimitTest.to_platform(:INFINITY, nil, apart)
  assert_equal "18446744073709551614", ProcessRlimitTest.to_platform(:SAVED_CUR, nil, apart)
  assert_equal top, ProcessRlimitTest.to_platform(:SAVED_MAX, nil, apart)
  assert_equal "18446744073709551612", ProcessRlimitTest.to_platform(:VALUE, "18446744073709551612", apart)
  assert_equal "0", ProcessRlimitTest.to_platform(:VALUE, "0", apart)
  # A number the platform spells another kind as would be set as that kind.
  assert_nil ProcessRlimitTest.to_platform(:VALUE, "18446744073709551613", apart)
  assert_nil ProcessRlimitTest.to_platform(:VALUE, "18446744073709551614", apart)
  assert_nil ProcessRlimitTest.to_platform(:VALUE, top, apart)

  # Where all three are spelled alike, that one number is refused as a count.
  alike = [top, "9223372036854775807", "9223372036854775807", "9223372036854775807"]
  assert_nil ProcessRlimitTest.to_platform(:VALUE, "9223372036854775807", alike)
  assert_equal top, ProcessRlimitTest.to_platform(:VALUE, top, alike)

  # A saved limit the platform does not name has no number, and the numbers
  # it would have had are counts.
  unnamed = [top, top, nil, nil]
  assert_nil ProcessRlimitTest.to_platform(:SAVED_CUR, nil, unnamed)
  assert_nil ProcessRlimitTest.to_platform(:SAVED_MAX, nil, unnamed)
  assert_equal "18446744073709551614", ProcessRlimitTest.to_platform(:VALUE, "18446744073709551614", unnamed)

  # A number wider than the platform's type has none either.
  narrow = ["4294967295", "4294967295", nil, nil]
  assert_equal "4294967294", ProcessRlimitTest.to_platform(:VALUE, "4294967294", narrow)
  assert_nil ProcessRlimitTest.to_platform(:VALUE, "4294967296", narrow)
end

assert('Process.setrlimit refuses the number the platform spells no limit as') do
  skip "this port declares no getrlimit" unless ProcessTestUtil.getrlimit?
  skip "this port declares no setrlimit" unless ProcessTestUtil.setrlimit?
  skip "the platform's limits are not compared here" unless ProcessRlimitTest.respond_to?(:native_infinity)

  cur, max = Process.getrlimit(:CORE)
  # A hard limit of no limit already, so that a number let through as no limit
  # would be set rather than refused by the platform for raising one.
  skip "this process may not leave a core file of any size" unless max == Process::RLIM_INFINITY
  infinity = ProcessRlimitTest.native_infinity
  skip "this build's Integer cannot write #{infinity}" unless ProcessClockTest.fits?(infinity)

  begin
    assert_raise(Errno::EINVAL) { Process.setrlimit(:CORE, 0, infinity.to_i) }
  ensure
    Process.setrlimit(:CORE, cur, max)
  end
  assert_equal [cur, max], Process.getrlimit(:CORE)
...
assert('the limit calls reach the resource the platform calls by that name') do
  skip "this port declares no getrlimit" unless ProcessTestUtil.getrlimit?
  skip "the platform's limits are not compared here" unless ProcessRlimitTest.respond_to?(:native_is?)

  # The constant is looked up by the name, and the resource the HAL reads and
  # writes for it is compared against the one <sys/resource.h> calls by that
  # name, both limits moved in a child through setrlimit(2) and read back.
  # Neither side goes through the other's table.
  compared = 0
  ProcessTestUtil.rlimit_resources.each do |name|
    next unless Process.const_defined?("RLIMIT_#{name}")
    answer = ProcessRlimitTest.native_is?(Process.const_get("RLIMIT_#{name}"), name)
    next if answer.nil?
    assert_true answer, name
    compared += 1
  end
