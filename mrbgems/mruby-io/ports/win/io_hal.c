...
/* The permission bits, this host's to the HAL's.  The C runtime names
   three, _S_IREAD, _S_IWRITE and _S_IEXEC, and they are the owner's: its
   _chmod(), _umask() and _open() read those and no others, so a mode handed
   down is read for its owner bits alone.  `File.umask(0022)` must leave the
   owner writing; collapsing any write bit into _S_IWRITE would hand _umask()
   the one bit that takes writing away.  What _stat() reports for group and
   other is a mirror of the owner's, which convert_stat() makes for itself. */
static mrb_int
