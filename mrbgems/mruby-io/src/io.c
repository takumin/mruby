 ...
 * taking their descriptors over, and the arguments are left closed.  This is
...
io_close_write(mrb_state *mrb, mrb_value io)
{
  ...
  if (fd2 == -1) {
    ...
    if (fptr->readable && mrb_nil_p(mrb_iv_get(mrb, io, MRB_IVSYM(__pid)))) {
...
static const mrb_mt_entry io_rom_entries[] = {
  ...
  /* Likewise, since closing the write end of a stream opened to a child can
     be closing the whole stream, and then the child owes the same wait. */
  MRB_MT_ENTRY(io_close_write,       MRB_SYM(_close_write),  MRB_ARGS_NONE()),
