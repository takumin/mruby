...
main(int argc, char **argv)
{
 ...
 l_restart:
  ...
  /* How the program ended, taken before anything below can disturb it.  The
     debugger's own command loop follows and used to decide the exit status
     by itself -- an unconditional 0 -- so `mrdb prog.rb` answered success for
     a program that raised, and for one that would not even compile, where
