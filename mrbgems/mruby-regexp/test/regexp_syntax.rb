...
assert("Regexp - a group name may not hold a ')'") do
  ...
  # a ')' landed inside the name -- and a ')' inside a name is a ')' to the
  # free-spacing prescan, which walks the pattern without reading names.
  msg = "invalid group name"
  ...
  # and the group the ')' used to close stays open: the free-spacing scope
  # the group opened no longer ends inside its own name
  assert_raise(RegexpError) { Regexp.new("(?<a)b>(?x)c d)e f") }
end
