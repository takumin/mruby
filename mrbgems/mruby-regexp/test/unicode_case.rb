...
assert("Regexp - Unicode case folding under /i") do
  ...
  # Byte escapes that spell a character are that character, so they fold as
  # the character does, in the class as in the literal path.
  ...
  # what the class takes there is one byte where the character class takes the
  # character the fold reached.
  assert_equal 2, "ā".match(Regexp.new("[\\xC4\\x80]", Regexp::IGNORECASE))[0].bytesize
