...
class String
  ...
  def scan(pattern)
    ...
    # answers for it, which is the way `gsub` does: one that changed the
    # length is refused with `RuntimeError` after it returns, the next match
    # is searched for in the string it left, and the match left in $~ is a
    # search once more from the offset the last match was found from, on the
    # string as it stands at the end. That search also republishes what the
    # failed one that ends the loop clears; a scan that matched nothing keeps
    # the cleared state.
    last = nil
    while pos <= len
    ...
    Regexp.__byte_search(pattern, self, last) if last
    self
  end
