...
/* What one bt_match() call answers. BT_MATCH is the search having reached
   ...
   BT_LIMIT is the search giving up at one of the limits before it had either.
   A limit says nothing about the text, so it is never read as a branch having
   failed and answered from the alternatives left: that would be answering a
   smaller question with whatever they produce -- a shorter match, a later one
   or none, told from the real answer by nothing. backtrack_exec() stops the
   whole search at it (see there). */
#define BT_FAIL 0
#define BT_MATCH 1
#define BT_LIMIT 2
...
typedef struct {
  ...
  /* The choice points not yet tried and the writes not yet taken back, both
     grown on the heap. A search starts with neither allocated: a pattern that
     never forks never pays for them. */
  re_cpoint *cp;
  uint32_t cp_top;
  uint32_t cp_capa;
  re_undo *undo;
...
/* Whether the search may hold one more entry of backtracking state.
   ...
   to take back, and bounding their sum is what bounds the memory a search on
   the heap can ask for. */
static mrb_bool
...
/* Push a choice point. FALSE is the search giving up at the stack limit,
   which is what a refused allocation is read as too: growing with
...
bt_push(bt_state *m, const char *sp, uint32_t pc, uint8_t kind, uint32_t group)
{
  ...
  if (m->cp_top == m->cp_capa) {
    uint32_t capa = m->cp_capa ? m->cp_capa * 2 : 16;
...
bt_match(bt_state *m, const char *sp, uint32_t pc)
{
  ...
  for (;;) {
      ...
      if (inst.a) {
        if (inst.offset > pc) {
          if (!bt_push(m, sp, inst.offset, RE_CP_FORK, 0) ||
              !bt_iter_begin(m, pc, sp)) return BT_LIMIT;
          pc++;
          break;
        }
        if (ITER_EMPTY(m, pc, sp)) { pc++; break; }
        if (!bt_push(m, sp, inst.offset, RE_CP_ITER, pc)) return BT_LIMIT;
        pc++;
        break;
      }
      if (!bt_push(m, sp, inst.offset, RE_CP_FORK, 0)) return BT_LIMIT;
      pc++;
      break;
    ...
    case RE_SPLITNG:
      ...
      if (inst.a) {
        if (inst.offset > pc) {
          if (!bt_push(m, sp, pc + 1, RE_CP_ITER, pc)) return BT_LIMIT;
          pc = inst.offset;
          break;
        }
        if (ITER_EMPTY(m, pc, sp)) { pc++; break; }
      ...
      if (!bt_push(m, sp, pc + 1, RE_CP_FORK, 0)) return BT_LIMIT;
      pc = inst.offset;
      break;
    ...
    case RE_SAVE:
      {
        ...
        if (!bt_log(m, &captures[slot], (int)(sp - str))) return BT_LIMIT;
        pc++;
      }
      break;
    ...
    case RE_NEG_LOOKBEHIND:
      {
        ...
        if (!bt_push(m, sp, inst.offset, negated ? RE_CP_NEG : RE_CP_BARRIER,
                     pat->code[inst.offset - 1].offset)) {
          return BT_LIMIT;
        }
        m->pass = ++m->pass_seq;
        sp = from;
    ...
    case RE_ATOMIC:
      ...
      if (!bt_push(m, sp, pc + 1, RE_CP_BARRIER, inst.offset)) return BT_LIMIT;
      pc++;
      break;
  ...
  fail:
    ...
    for (;;) {
      ...
      sp = c.sp;
      pc = c.pc;
      if (c.kind == RE_CP_ITER && !bt_iter_begin(m, c.group, sp)) return BT_LIMIT;
      break;
    }
  }
}
