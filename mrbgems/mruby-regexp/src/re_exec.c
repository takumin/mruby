...
/* What one bt_match() frame answers. The frame that reaches RE_MATCH answers
   ...
   it is the search's answer rather than the frame's: every frame hands it up
   unchanged, as it does a cut, and backtrack_exec() stops at it (see there).
   A limit says nothing about the text, so no frame may read it as its branch
   having failed and answer with its other branch, which would be answering a
   smaller question with whatever the alternatives inside the limit produce: a
   shorter, a later or no match, told from the real answer by nothing. */
#define BT_FAIL 0
#define BT_MATCH 1
#define BT_LIMIT 2
