...
/* Pre-interned symbols for $~, $&, $`, $' and $+ (cached on first use).
   $~ sits with them rather than being interned where it is written: it is
   written on every search, hit or miss, and interning it there asked the
   symbol table for a name already in hand. */
enum { BACKREF, LAST_MATCH, PRE_MATCH, POST_MATCH, LAST_PAREN, LAST_SYM_COUNT };
...
ensure_match_syms(mrb_state *mrb)
{
  ...
  last_match_syms[BACKREF] = mrb_intern_lit(mrb, "$~");
...
set_match_globals(mrb_state *mrb, mrb_value obj, mrb_value str, int *captures, int num_captures)
{
  ...
  mrb_gv_set(mrb, last_match_syms[BACKREF], obj);
