        ...
        if (c->p[1] == ':') {
          ...
          while (peek(c) != close && peek(c) >= 0) {
            /* CRuby's fetch_name() ends the scan at a ')' instead of taking it
               ...
               name`. The first byte is exempt -- the test starts one byte in,
               so (?<)>x) names ")" in both engines. Taking it was not only a
               message difference: a ')' inside a name is a ')' to every
               prescan that walks the pattern without reading names, so the
               free-spacing pass closed a scope this group had opened. The
               message quotes to the end of the pattern, as CRuby does when
               the scan never reached a delimiter. */
