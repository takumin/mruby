...
/* Read the `{...}` of a property escape, the cursor standing on the `{`.
   ...
   The name is folded into `buf` the way the tables hold it -- lower case,
   without the underscores, hyphens and spaces that are not part of a name --
   and the length it comes to is returned, which can be past the buffer for a
