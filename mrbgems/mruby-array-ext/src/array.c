...
/* Answers one question -- "is this value one of the elements I hold?" -- and
   ...
   functions until #112, and `Array#intersection` narrowed by the union of its
   arguments on one side and by each argument on the other. A caller states
   its algorithm once, against ary_memb_has(), and never learns which
   strategy replied. */
...
/* Runs `body` with the set destroyed even if it raises. The walk owns nothing
   ...
   that mrb_protect_error() sets up -- the walk is picked exactly when the
   work is too small to absorb that. */
