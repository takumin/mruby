...
- **_NOTE_**: a preallocated symbol's ID is a hash of its name rather than its
  ...
  `ccache`/`sccache` keep the objects they hold. `MRB_PRESYM_MAX` is now the
  size of the ID range and not the number of symbols; `mrb_presym_count()` and
  `mrb_presym_at()` report what it used to. The name table that went with the
  old numbering is gone too, so `libmruby` loses about 8KB and one relocation
  per preallocated symbol
